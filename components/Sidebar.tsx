...
import { FaTools, FaLayerGroup, FaChartBar, FaCog, FaChevronLeft, FaChevronRight, FaDatabase } from 'react-icons/fa';
    ...
    <div className={`${isOpen ? 'w-64' : 'w-20'} h-screen bg-slate-900 flex flex-col transition-all duration-300 relative border-r border-slate-800`}>
      ...
      <div className="p-4 border-t border-slate-800">
        <div className={`flex items-center gap-3 ${!isOpen && 'justify-center'}`}>
          ...
          {isOpen && <span className="text-xs text-slate-500 uppercase tracking-widest">{dbConnected ? 'Online' : 'Offline'}</span>}
        </div>
      </div>
    </div>
  );
